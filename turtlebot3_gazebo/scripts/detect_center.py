...
def detect_cube_position(frame, camera_matrix, dist_coeffs, marker_length, cube_size):
    ...
    result = least_squares(
        ...
        initial_params,
        args=(observations,),
        method='trf',  # Levenberg-Marquardt算法
        max_nfev=200  # 最大迭代次数
    )
...
class ImageReceiver:
    def __init__(self):
        ...
        self.camera_info_sub = rospy.Subscriber("/tb3_1/camera/rgb/camera_info", CameraInfo, self.camera_info_callback)
        self.image = None
        self.camera_matrix = []
        self.dist_coeffs = []
        self.bridge = CvBridge()

    def image_callback(self, std_msg:Image):
        self.image = self.bridge.imgmsg_to_cv2(std_msg, desired_encoding='rgb8')
        # print(np.shape(self.image))
        # cv2.imshow("CV Image", self.image)
        # cv2.waitKey(1)

    def camera_info_callback(self, std_msg:CameraInfo):
        self.camera_matrix = np.reshape(std_msg.K, (3, 3))
        self.dist_coeffs = std_msg.D
    
    def run(self):
        while not rospy.is_shutdown():
            if self.image is not None and self.camera_matrix is not [] and self.dist_coeffs is not []:
                # 检测立方体位置
                T, R = detect_cube_position(
                    self.image,
                    receiver.camera_matrix,
                    receiver.dist_coeffs,
